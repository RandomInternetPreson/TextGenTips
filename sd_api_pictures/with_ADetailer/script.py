...
params = {
    ...
    'prompt_prefix': 'detailed',
    'negative_prompt': 'cartoon, blurry, distorted, cgi',
    'width': 1024,
    'height': 1024,
    ...
    'steps': 32,
    'cfg_scale': 10.5,
    'textgen_prefix': 'detailed, photorealisitc',
    'sd_checkpoint': ' ',
    'checkpoint_list': [" "]
}
...
def triggers_are_in(string):
    ...
    # return bool(re.search('(?aims)(send|mail|message)\\b.+?\\b(image|pic(ture)?|photo|snap(shot)?|selfie|meme|prompt)s?\\b', string))
    return bool(re.search('(?aims)(send|mail|message)\\b.+?\\b(selfie|meme|prompt)s?\\b', string))
...
def input_modifier(string):
    ...
    if triggers_are_in(string):  # if we're in it, check for trigger words
        ...
        #else:
            #string = params['textgen_prefix'].replace("[subject]", "a description of what the user requested, limited to 2 sentences using the picture examples provided to you ")
...
def get_SD_pictures(description, character):
    ...
    payload = {
        "prompt": params['textgen_prefix'] + description,
        "alwayson_scripts": {
            "ADetailer": {
                "args": [
                {
                    "ad_model": "face_yolov8n.pt",
                    "ad_prompt": params['prompt_prefix'],
                    "ad_inpaint_width": 1024,
                    "ad_inpaint_height": 1024,
                    "ad_negative_prompt": 'drawing, unrealistic eyeballs, cartoon, blurry eyes, ugly, perfect skin'
                }          
               ]
              }
             },
    ...
    for img_str in r['images']:
        if params['save_img']:
            ...
            image = Image.open(io.BytesIO(img_data)) ##new
            image = image.resize((800, 800)) ##new

            variadic = f'{date.today().strftime("%Y_%m_%d")}/{character}_{int(time.time())}'
            output_file = Path(f'extensions/sd_api_pictures/outputs/{variadic}.png')
            #output_file = Path(f'Z:/outputs/{variadic}.png')
            #output_file = Path(f'Z:/{variadic}.png')
            output_file.parent.mkdir(parents=True, exist_ok=True)

            with open(output_file.as_posix(), 'wb') as f:
                #f.write(img_data)
                image.save(f, format="PNG") ##new
...
def ui():
    ...
    with gr.Accordion("Parameters", open=True, elem_classes="SDAP"):
        ...
        with gr.Accordion("Generation parameters", open=False):
            prompt_prefix = gr.Textbox(placeholder=params['prompt_prefix'], value=params['prompt_prefix'], label='Prompt Prefix Used for ADetailer Input')
            #prompt_prefix = gr.Textbox(placeholder=params['prompt_prefix'], value=params['prompt_prefix'], label='Prompt Prefix (best used to describe the look of the character)')
            textgen_prefix = gr.Textbox(placeholder=params['textgen_prefix'], value=params['textgen_prefix'], label='textgen prefix what is sent at the beginning of description')
